...
void recurse_out(heap_t *tree);
heap_t *nth(heap_t *tree);
...
/**
 * recurse_out - Recursively extracts the max from the tree.
 *
...
void recurse_out(heap_t *tree)
{
	...
	if (!tree->left)
		return;
	sub_max = nth((tree)->left);
	if (tree->right)
		right_max = nth(tree->right);
...
/**
 * out - Finds the maximum node in a tree.
 *
 * @tree: The pointer to the root of the tree.
 *
